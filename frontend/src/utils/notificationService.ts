// Vertex Notification Service using NTFY.sh
...
export class VertexNotificationService {
   ...
   */
  static async sendDailySchedule(userTopic: string, schedule: any[]): Promise<boolean> {
    ...
    return this.sendNotification({
      topic: userTopic,
      title: '🗓️ Today\'s Schedule - Vertex',
  ...
  static async sendAttendanceAlert(userTopic: string, subject: string, percentage: number): Promise<boolean> {
    return this.sendNotification({
      topic: userTopic,
      title: '⚠️ Low Attendance Alert - Vertex',
      ...
      priority: 'high',
      tags: ['skull', 'attendance', 'alert', 'vertex'],
  ...
  static generateUserTopic(userId: string): string {
    return `vertex-user-${userId.replace(/[^a-zA-Z0-9]/g, '')}`;
  }
...
export class VertexNotificationScheduler {
