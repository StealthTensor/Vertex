...
const PercentagePage = () => {
  ...
  const withCourseMeta = marks.map((m) => {
    ...
    return {
      ...
      type: m.category,
      percent,
      obtained: m.total?.obtained || 0,
      max: m.total?.maxMark || 0,
    };
  });
    ...
    <div className="flex flex-col gap-6 py-2 ">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 px-2 lg:px-5">
        <div className="rounded-xl apply-border-md bg-white/5 p-4 text-center">
          ...
          <div className="text-white text-2xl font-semibold">{totalObtained} / {totalMax}</div>
        </div>
        <div className="rounded-xl apply-border-md bg-white/5 p-4 text-center">
          <div className="text-white/60 text-sm">Total Percentage</div>
          <div className="text-white text-2xl font-semibold">{totalPercentage.toFixed(2)}%</div>
        </div>
        <div className="rounded-xl apply-border-md bg-white/5 p-4 text-center">
        ...
        {withCourseMeta.map((item) => (
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-white">{item.obtained} / {item.max}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-white/70 text-sm">Percentage</span>
              <span className={item.percent >= 75 ? "text-green-400" : "text-yellow-300"}>{item.percent.toFixed(2)}%</span>
            </div>
          </div>
        ))}
      </div>
